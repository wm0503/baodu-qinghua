...
def main(config):
    ...
    if config["model_type"] in ["bert", "bert_lstm", "bert_cnn", "bert_mid_layer"]:
        message = "Config:" + " " + config["model_type"] + ("  lr:%.0e" % config["learning_rate"]).replace("0",
                                                                                                           "") + "  batch_size:%d  " % \
                  config["batch_size"] + config["pooling_style"]
    else:
        message = "Config:" + " " + config["model_type"] + ("  lr:%.0e" % config["learning_rate"]).replace("0",
                                                                                                           "") + "  hidden_size:%d" % \
                  config["hidden_size"] + "  batch_size:%d  " % config["batch_size"] + config["pooling_style"]
...
if __name__ == "__main__":
    ...
    # 训练bert系列模型
    headers = ["model", "learning_rate", "batch_size", "pooling_style", "acc", "time(预测100条耗时)"]
    datas = []
        ...
        for lr in [1e-3, 1e-5]:
            ...
            for batch_size in [64, 128]:
                ...
                for pooling_style in ["avg", "max"]:
                    ...
                    acc, time = main(Config)
                    print("最后一轮准确率：", acc, "当前配置：", Config)
                    datas.append({"model": Config["model_type"],
                                  "learning_rate": ("%.0e" % Config["learning_rate"]).replace("0", ""),
